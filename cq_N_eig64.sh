#!/bin/bash
#SBATCH --job-name=cq_N_eig64
#SBATCH --array=0-3                    # 1 mode × 4 mx × 1 my = 4 jobs
...
MODE_INDEX=0                          # always 0, since only one mode
MX_INDEX=${SLURM_ARRAY_TASK_ID}       # 0–3
MY_INDEX=0                             # always 0, only one value

MODE=${MODES[$MODE_INDEX]}
MX=${MX_VALUES[$MX_INDEX]}
MY=${MY_VALUES[$MY_INDEX]}

OUT_DIR="results_cq_Neig64/${MODE}/mx${MX}_my${MY}"
mkdir -p "$OUT_DIR"
