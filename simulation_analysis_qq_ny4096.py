...
def scan_all_data(root_dir):
    ...
    """
    mode_dir = os.path.join(root_dir, "quantum")
    results = {}
    if not os.path.isdir(mode_dir):
        print(f"[WARN] Directory not found: {mode_dir}")
        return results
    ...
    for sub in os.listdir(mode_dir):
        ...
        if mx is None or my is None:
            continue
        try:
            data, params = load_simulation_data(subpath, "quantum")
            results[(mx, my)] = {"quantum": (data, params)}
        except Exception as e:
            print(f"[WARN] quantum load failed at {subpath}: {e}")
    return results
...
def main():
    root = "/Users/doyeonkim/OneDrive/Documents/Project1_Sanjeev/Three_Mode_VaryingMxMy_May23"
    panel_dir = os.path.join(root, "panels_quantum_only")
    os.makedirs(panel_dir, exist_ok=True)
    #summary_dir = os.path.join(root, "entropy_energy_by_mx")
    ##os.makedirs(summary_dir, exist_ok=True)

    aall_data = scan_quantum_data_only(root)
